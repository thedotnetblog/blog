...
What should teams building internal coding agents copy?

Define quality guardrails upfront, then optimize for latency and cost under those constraints. Measure both median and tail behavior. The p95 improvements in time-to-first-edit and token usage are often more valuable than p50 wins for real user satisfaction.

Also, avoid overfitting to offline evals alone. The VS Code team used offline checks, then validated on live traffic before rollout. That order matters because real workflows expose behaviors synthetic benchmarks miss.
