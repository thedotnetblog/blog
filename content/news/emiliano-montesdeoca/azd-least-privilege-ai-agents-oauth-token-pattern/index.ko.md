---
title: "당신의 AI 에이전트에는 신원 문제가 있습니다 (그리고 이것이 그것을 해결하는 템플릿입니다)"
date: 2026-05-20
author: "Emiliano Montesdeoca"
description: "Curity와 Microsoft의 새로운 azd 템플릿은 세밀한 스코프의 단기 OAuth 토큰을 사용하는 AI 에이전트를 구축하는 방법을 보여줍니다 — 에이전트가 절대 보아서는 안 되는 데이터를 볼 수 없도록 하기 위해."
tags:
  - Azure Developer CLI
  - AI
  - Security
...
대부분의 에이전트 샘플은 사용자 인증을 잘 처리합니다. 인가는 잘 처리하지 못합니다. 사용자가 *누구*인지 아는 것이 그들이 봐야 할 *데이터*가 무엇인지 알려주지는 않습니다.
...
1. **첫 번째 교환** — 스코프를 좁히고 불투명한 토큰을 JWT로 변환
2. **두 번째 교환** — 에이전트 신원과 MCP 서버 홉을 위한 새 오디언스 추가
...
`customer_id`는 에이전트가 제어하는 매개변수로 전달되는 것이 아니라 인가 서버에 의해 토큰에 내장됩니다. API는 에이전트의 지시가 아닌 토큰을 확인합니다.
...
원본 게시물: [Least privilege AI agents: A new azd template from Curity and Microsoft](https://devblogs.microsoft.com/azure-sdk/azd-curity-least-privilege-ai-agents/)
