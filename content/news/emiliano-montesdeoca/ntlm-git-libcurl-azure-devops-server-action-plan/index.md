...
First, verify current auth behavior now. Run trace-based checks and ticket cache validation in real developer and build-agent contexts, including off-domain and remote-network paths. Second, fix Kerberos end-to-end: SPNs, DNS aliases, load balancer settings, delegation, and domain controller reachability. Third, identify non-domain-joined or workgroup scenarios early and design an SSH lane where Kerberos cannot be made reliable.
