---
title: "Governar les crides d'eines MCP a .NET amb l'Agent Governance Toolkit"
date: 2026-05-04
author: "Emiliano Montesdeoca"
description: "Resum practic per a equips .NET sobre \"Governar les crides d'eines MCP a .NET amb l'Agent Governance Toolkit\", amb passos clars per avaluar-ho en produccio."
tags:
  - .NET
  - MCP
  - AI Agents
  - Security
---

*Aquest article s'ha traduit automaticament. Per a la versio original, [fes clic aqui]({{< ref "index.md" >}}).*

[Governar les crides d'eines MCP a .NET amb l'Agent Governance Toolkit](https://devblogs.microsoft.com/dotnet/governing-mcp-tool-calls-in-dotnet-with-the-agent-governance-toolkit/) es rellevant per a equips que construeixen i operen sistemes .NET en produccio.

Des del meu punt de vista, el mes important no es nomes la novetat, sino com convertir-la rapidament en una practica d'enginyeria repetible.

## Per que importa per als equips .NET

Aquest canvi ajuda a equilibrar velocitat d'entrega, consistencia de plataforma i governanca.

## Seguents passos practics

1. Valida la funcionalitat en un pilot .NET petit amb dades realistes.
2. Defineix observabilitat i un pla de rollback abans d'escalar.
3. Documenta el patro per reutilitzar-lo en altres equips.

## Font

- Article original: [https://devblogs.microsoft.com/dotnet/governing-mcp-tool-calls-in-dotnet-with-the-agent-governance-toolkit/](https://devblogs.microsoft.com/dotnet/governing-mcp-tool-calls-in-dotnet-with-the-agent-governance-toolkit/)
